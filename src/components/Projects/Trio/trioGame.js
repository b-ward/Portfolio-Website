...
class TrioGame extends Component {
  ...
  pile_refresh() {
    ...
    var card_changes = 0;
    var pile_card_pos = 0
    this.state.pile_card_to_get = this.state.deck[0];
    while (card_changes === 0) {
...
//   //Performed as the page loads
  ...
  //When the player clicks the face-down card the pick up the card and add it to their hand
    pick_up_deck() {
      if (this.state.picked_up === false) {
        ...
        this.hand_refresh();
        this.state.picked_up = true;
        this.state.picked_up_deck = true;
        this.move_cards_pos()
        //Displays instructions only if the player allows
    ...
    pick_up_pile() {
      ...
      if (this.state.picked_up === false){
        ...
        this.hand_refresh();
        this.state.picked_up = true;
        this.state.picked_up_pile = true;
        this.move_cards_pos()
        //Displays instructions only if the player allows
        if (this.state.instruction_button === true){window.alert("Now you can either play two trios (if you have the correct cards) by pressing the 'PLAY TRIOS' button, or you can discard a card by clicking the 'DISCARD' button.")}
    }}
    ...
    add_trio_1() {
      ...
      this.state.trio_to_add_to = 1;}
    }
    add_trio_2() {
      if (this.state.add_to_trios === true){
      //Displays instructions only if the player allows
      if (this.state.instruction_button === true){window.alert('Click the card you would like to add to this trio')}
      this.state.trio_to_add_to = 2;}
    }
    add_trio_3() {
      if (this.state.add_to_trios === true){
      //Displays instructions only if the player allows
      if (this.state.instruction_button === true){window.alert('Click the card you would like to add to this trio')}
      this.state.trio_to_add_to = 3;}
    }
    add_trio_4() {
      if (this.state.add_to_trios === true){
      //Displays instructions only if the player allows
      if (this.state.instruction_button === true){window.alert('Click the card you would like to add to this trio')}
      this.state.trio_to_add_to = 4;}
    }
  ...
  player_discard(){
    this.state.add_to_trios = false;
    this.state.discarded_card = this.state.hand[this.state.card_num_clicked];
    this.state.hand.splice(this.state.card_num_clicked,1);
    this.hand_refresh();
    if (this.state.picked_up_deck === true){this.state.deck.splice(0,1); this.state.picked_up_deck = false}
    if (this.state.picked_up_pile === true){this.state.picked_up_pile = false}
    this.state.deck.unshift(this.state.discarded_card);
    this.pile_refresh(); 
    this.state.discard = false;
    console.log(this.state.hand);
    console.log(this.state.deck);
    this.CPU_turn();
    this.state.picked_up = false;
  }
  ...
  add_cards(trio, trio_list) {
    if (trio.length > 2){
      ...
      //In order to check if the move is valid, the card played must be compared to an actual numbererd card, rather than a 'wild' joker card
      if (card_to_check[0] === 'S'){card_to_check = trio[1]}
      if (card_to_check[0] === 'S'){card_to_check = trio[2]; this.state.all_jokers = true}
      ...
      if (this.state.hand[this.state.card_num_clicked][0] === card_to_check[0] || this.state.all_joker === true || this.state.hand[this.state.card_num_clicked][0] === 'S'){
        ...
        this.state.amount_discarded = this.state.amount_discarded -1;
        //Displays instructions only if the player allows
    ...
    //If the move is not valid a message appears
      ...
      this.state.all_joker = false
      this.state.add_to_trios = false
      this.state.trio_to_add_to = 0
  }
      ...
      if (this.state.playing_trios === true){
        ...
        if (this.state.correct === 0) {
          ...
          if (this.state.trio_played_1.length === 3) {
            ...
            if (this.state.trio_played_1[0][0] === 'S'){if (this.state.trio_played_1[1][0] === this.state.trio_played_1[2][0]){this.state.correct = 1}}
            else if (this.state.trio_played_1[1][0] === 'S'){if (this.state.trio_played_1[0][0] === this.state.trio_played_1[2][0]){this.state.correct = 1}}
            else if (this.state.trio_played_1[2][0] === 'S'){if (this.state.trio_played_1[0][0] === this.state.trio_played_1[1][0]){this.state.correct = 1}}

            else if (this.state.trio_played_1[0][0] === 'S' && this.state.trio_played_1[1][0] === 'S') {this.state.correct = 1}
            else if (this.state.trio_played_1[1][0] === 'S' && this.state.trio_played_1[2][0] === 'S') {this.state.correct = 1}
            else if (this.state.trio_played_1[0][0] === 'S' && this.state.trio_played_1[2][0] === 'S'){this.state.correct = 1}
            else if (this.state.trio_played_1[0][0] === 'S' && this.state.trio_played_1[1][0] === 'S' && this.state.trio_played_1[2][0] === 'S'){this.state.correct = 1}

            else if (this.state.trio_played_1[0][0] === this.state.trio_played_1[1][0] && this.state.trio_played_1[1][0] === this.state.trio_played_1[2][0] && this.state.trio_played_1[0][0] === this.state.trio_played_1[2][0]){this.state.correct = 1}

            //A message is displayed if the trio is invalid and the cards played and put back into the player's hand
            else {window.alert('Sorry that is not a valid trio'); this.state.hand.push(this.state.trio_played_1[0]);this.state.hand.push(this.state.trio_played_1[1]);this.state.hand.push(this.state.trio_played_1[2]); this.state.trio_played_1 = []; document.getElementById('player_trio_1_card_1').src = '../Trio/Images/blank.png'; document.getElementById('player_trio_1_card_2').src = '../Trio/Images/blank.png'; document.getElementById('player_trio_1_card_3').src = '../Trio/Images/blank.png'; this.hand_refresh();this.state.playing_trios = false;}}}
        ...
        //If the first trio has been completed but the second has not - Same thing as above occurs except the second trio is checked
          ...
          if (this.state.trio_played_2.length === 3) {
                  if (this.state.trio_played_2[0][0] === 'S'){if (this.state.trio_played_2[1][0] === this.state.trio_played_2[2][0]){this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise(); }}
                  else if (this.state.trio_played_2[1][0] === 'S'){if (this.state.trio_played_2[0][0] === this.state.trio_played_2[2][0]){this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}}
                  else if (this.state.trio_played_2[2][0] === 'S'){if (this.state.trio_played_2[0][0] === this.state.trio_played_2[1][0]){this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}}

                  else if (this.state.trio_played_2[0][0] === 'S' && this.state.trio_played_2[1][0] === 'S') {this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}
                  else if (this.state.trio_played_2[1][0] === 'S' && this.state.trio_played_2[2][0] === 'S') {this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}
                  else if (this.state.trio_played_2[0][0] === 'S' && this.state.trio_played_2[2][0] === 'S') {this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}
                  else if (this.state.trio_played_2[0][0] === 'S' && this.state.trio_played_2[1][0] === 'S' && this.state.trio_played_2[2][0] === 'S') {this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}

                  else if (this.state.trio_played_2[0][0] === this.state.trio_played_2[1][0] && this.state.trio_played_2[1][0] === this.state.trio_played_2[2][0] && this.state.trio_played_2[0][0] === this.state.trio_played_2[2][0]){this.state.player_trios_complete = true; this.state.correct = 2; this.discard_initialise();}
            ...
            //All cards from the first and second trios are added back to the player's hand
              ...
              this.state.playing_trios = false;
  ...
  //Occurs of the player has already played their two trios
    ...
    if (this.state.hand.length === 0) {window.alert('YOU WIN'); this.state.game_over = true; window.location.href='/Projects'}
  }

  ///////////////////////////////////////////////////////////////////  REARRANGE CARDS IN PLAYER'S HAND ////////////////////////////////////////////////////////
  if (this.state.rearrange === true){
    if (this.state.card_swap_count === 0){this.state.first_card_to_swap = this.state.card_num_clicked; this.state.card_swap_count += 1;}
    else if (this.state.card_swap_count === 1){
      this.state.second_card_to_swap = this.state.card_num_clicked
      ...
      this.state.card_swap_count = 0
      this.state.rearrange = false
      this.hand_refresh()
    }}
  }

  //Functions that indicate which card has been clicked
    discard1() {if (this.state.game_over === false) {this.state.card_num_clicked = 0; this.card_clicked();}}
    discard2() {if (this.state.game_over === false) {this.state.card_num_clicked = 1; this.card_clicked();}}
    discard3() {if (this.state.game_over === false) {this.state.card_num_clicked = 2; this.card_clicked();}}
    discard4() {if (this.state.game_over === false) {this.state.card_num_clicked = 3; this.card_clicked();}}
    discard5() {if (this.state.game_over === false) {this.state.card_num_clicked = 4; this.card_clicked();}}
    discard6() {if (this.state.game_over === false) {this.state.card_num_clicked = 5; this.card_clicked();}}
    discard7() {if (this.state.game_over === false) {this.state.card_num_clicked = 6; this.card_clicked();}}
    discard8() {if (this.state.game_over === false) {this.state.card_num_clicked = 7; this.card_clicked();}}
    discard9() {if (this.state.game_over === false) {this.state.card_num_clicked = 8; this.card_clicked();}}
    discard10() {if (this.state.game_over === false) {this.state.card_num_clicked = 9; this.card_clicked();}}
    discard11() {if (this.state.game_over === false) {this.state.card_num_clicked = 10; this.card_clicked();}}
    discard12() {if (this.state.game_over === false) {this.state.card_num_clicked = 11; this.card_clicked();}}
    discard13() {if (this.state.game_over === false) {this.state.card_num_clicked = 12; this.card_clicked();}}
...
//     ////////////////////////////////////////////////CPU TURN/////////////////////////////////////////////////
    ...
    //If the CPU has two complete trios the trios are displayed on screen and the CPU has won
      if (this.state.CPU_trio_1.length === 3 && this.state.CPU_trio_2.length === 3){
        this.state.CPU_complete_trio_1 = this.state.CPU_trio_1
        this.state.CPU_complete_trio_2 = this.state.CPU_trio_2
        console.log('Made it 0')
          ...
          this.state.CPU_trios_complete = true;
                ...
                if (which_trio === 'S'){
                    if (this.state.trio_played_1.length > 0 && this.state.trio_played_1.length < 7){
                      ...
                      this.state.CPU_hand.splice(trio_adder,1)
                        ...
                        this.state.num_cards_left = this.state.num_cards_left - 1;
                        this.trio_refresh(this.state.trio_played_1, this.state.trio_list_1)
                        }
